...
ef_left  = pygame.Surface((30, HEIGHT),  pygame.SRCALPHA)
ef_right = pygame.Surface((30, HEIGHT),  pygame.SRCALPHA)
ef_top   = pygame.Surface((WIDTH, 30),   pygame.SRCALPHA)
ef_bot   = pygame.Surface((WIDTH, 30),   pygame.SRCALPHA)
...
# ======================== GAME STATE ========================
_SAVE = os.path.join(os.path.dirname(os.path.abspath(__file__)),'subway_save.json')
def save_data():
    try:
        with open(_SAVE,'w') as f:
            json.dump({'high_score':high_score,'coin_total':coin_total,'owned_skins':list(owned_skins),'current_skin':current_skin},f)
    except: pass
def load_data():
    try:
        if os.path.exists(_SAVE):
            with open(_SAVE,'r') as f: d=json.load(f)
            return d.get('high_score',0),d.get('coin_total',0),set(d.get('owned_skins',[0])),d.get('current_skin',0)
    except: pass
    return 0,0,{0},0

score = 0; high_score = 0
high_score, coin_total, owned_skins, current_skin = load_data()
...
while True:
        ...
        if not has_shield and not has_red_shield and not is_invincible:
            ...
            for obs in all_obs:
                if prect.colliderect(obs["rect"].inflate(-20,-20)):
                    ...
                    if lives<=0:
                        game_state="gameover"; high_score=max(high_score,score); save_data()
                    break
    ...
    if game_state in ("playing","gameover"):
        ...
        if edge_flash>0:
            ...
            for ef,pos in [(ef_left,(0,0)),(ef_right,(WIDTH-30,0)),(ef_top,(0,0)),(ef_bot,(0,HEIGHT-30))]:
                ef.fill((220,0,0,a))
                draw_surface.blit(ef,pos)
